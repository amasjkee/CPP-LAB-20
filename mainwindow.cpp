...
void MainWindow::saveImage() {
    ...
    QString fileName = QFileDialog::getSaveFileName(this, "Save Image", "",
                                                    "TIFF (*.tiff *.tif);;PNG (*.png);;JPEG (*.jpg);;BMP (*.bmp)");
    if (fileName.isEmpty()) {
        return;
    }

    // Если выбран TIFF - показываем диалог настроек
    if (fileName.toLower().endsWith(".tiff") || fileName.toLower().endsWith(".tif")) {
        bool isMono = TiffSaver::isMonochrome(processedImage);
        bool isGray = TiffSaver::isGrayscale(processedImage);

        TiffCompressionDialog dialog(isMono, isGray, this);
        if (dialog.exec() == QDialog::Accepted) {
            TiffSaveOptions options = dialog.getOptions();
            QString error;

            if (TiffSaver::saveTiff(fileName, processedImage, options, &error)) {
                statusBar()->showMessage("TIFF saved successfully", 3000);
            } else {
                QMessageBox::critical(this, "Error", "Failed to save TIFF: " + error);
            }
        }
    } else {
        // Обычное сохранение
        if (processedImage.save(fileName)) {
            statusBar()->showMessage("Image saved", 3000);
        } else {
